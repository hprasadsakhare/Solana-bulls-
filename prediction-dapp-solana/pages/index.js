...
const styles = {
  ...
  mainContainer: "w-7/8 space-x-5 h-full m-auto flex mt-20",
  leftMain: "flex flex-col w-3/4 h-full  p-6 overflow-y-scroll",
  portfolioAmountContainer: "flex w-full",
  portfolioAmount: "text-cyan-300 text-4xl",
  ...
  buyingPowerTitle: "text-cyan-200 font-bolder text-xl ",
  buyingPowerAmount:
    "text-white font-bolder text-xl flex flex-row items-center relative cursor-pointer",
  ...
  rightMain:
    "flex flex-col flex-5 h-[500px] bg-[#1E2123] mt-6 rounded-lg overflow-y-scroll",
  dropDownBets:
  ...
  button:
    "rounded-sm py-2 px-16 text-[#ffffff] text-xs border-[#30363b] bg-[#1E2123] border ",
  inputForm: "flex flex-row mt-4",
  input:
    "rounded-sm px-5 border-[#30363b] bg-[#1E2123] border mx-2 w-3/4 p-1 text-[#ffffff] focus:outline-none",
...
const Home = () => {
  ...
  const {connected ,
     masterAccount ,
      allBets ,
      createBet, } =useGlobalState()
  console.log(connected , masterAccount?.lastBetId.toString() , allBets , createBet)

  return (
    <>
    <Head>
    <title>Solana Bulls</title>
    </Head>
      ...
      <div className={styles.mainContainer}>
        <div className={styles.leftMain}>
          <div className={styles.portfolioAmountContainer}>
            ...
            <p className="text-cyan-200 w-full text-end text-2xl mt-2">CSP = {data.price} $</p>
          </div>
          <div>
          ...
          <div className={styles.buyingPowerContainer}>
            ...
            <div className={styles.buyingPowerTitle}>Create a Bet</div>
          </div>
          ...
          <form className="flex flex-col">
            <div className={styles.inputForm}>
              ...
              <p className="text-red-700 w-full text-center mt-3 font-bold">WARNING: The bet duration must be at least 300 sec.</p>
            <input
              type="submit"
              disabled={availableStock.length === STOCKDATA.length}
              value="Submit"
              className={`${styles.button
                }${" bg-green-700 text-lg cursor-pointer hover:bg-green-800 transition-all ease-in duration-100 text-center mt-8 self-center"}`}
              onClick={(e) => {
                e.preventDefault()
                createBet(
